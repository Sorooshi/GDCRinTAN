...
class GDCMEfn:
    """Computes various Gradient Descent Clustering Methods of the Embedded Feature-rich Networks (GDCMEfn).
    Parameters
    ----------
    ...
    tau : float, default=1e-2
        Regularization strength coefficient: $tau * norm(centroid_k)$
    ...
    @staticmethod
    def _get_subkey():
        seed = np.random.randint(low=0, high=int(1e8), size=1)[0]
    ...
    @staticmethod
    def softmax_fn(data_point):
        """ apply softmax function on the data_point vector"""
        return np.exp(data_point) / sum(np.exp(data_point))

    def compute_relations(self, data_points, chunk_size=1000):
        """returns the N*N relation/similarity array, called 'r', of the N*V features or N*N adjacency array."""
        n = len(data_points)
        if n >= chunk_size:  # 1000 is a reasonable array size. 
            print("Computing the relation of this large array will take a while")
            ...
            r = [data_points[chunk_size * i: chunk_size * (i + 1), :] @ data_points.T for i in range(chunks)]
            r = jnp.concatenate(r, axis=0)
            r = r / data_points.shape[1]  # normalizing to avoid numerical explosion
            # apply softmax
            for i in range(n):
                r = r.at[i, :].set(self.softmax_fn(data_point=r[i, :]))
        # compute the relation array in go for those array lower than 1000 data points.
        else:
            r = (data_points @ data_points.T) / data_points.shape[1]  # normalizing to avoid numerical explosion
            for i in range(n):
                r = r.at[i, :].set(self.softmax_fn(data_point=r[i, :]))
        return r
    ...
    def apply_gdc_rules(self, grads_x, grads_a, k, t):
        ...
        elif self.gdc_vanilla_reg:  # L2-norm regularization
            updated_centroid_x = self.centroids_x[k, :] - self.step_size * (
                    grads_x + (self.tau * jnp.abs(self.centroids_x[k, :]))
            )
            updated_centroid_a = self.centroids_a[k, :] - self.step_size * (
                    grads_a + (5 * self.tau * jnp.abs(self.centroids_a[k, :]))
            )
            self.centroids_x = self.centroids_x.at[k].set(updated_centroid_x)
            self.centroids_a = self.centroids_a.at[k].set(updated_centroid_a)
    ...
    def fit(self, x, a, distance_fn, y=None,):
        ...
        x : features array of shape (n_samples, n_features) or
             similarity/relation data of shape (n_samples, n_samples) instances to cluster.
        a : network adjacency array of shape (n_samples, n_samples)
            or similarity/relation data of shape (n_samples, n_samples) instances to cluster.
        ...
        elif self.update_rule == "vgdc_reg":
            self.gdc_vanilla_reg = True
        ...
        print(
            ...
            f"{self.gdc_vanilla, self.gdc_vanilla_reg, self.gdc_nestrov, self.gdc_adam},"
        )
        ...
        # To speed up the implementations "x" and "a" arrays are overwritten with their similarity/relation arrays.
        x = self.compute_relations(data_points=x)
        a = self.compute_relations(data_points=a)
        n_samples = x.shape[0]
        n_features = x.shape[1]
        self.n_samples = n_samples  # refactor it later (it was introduced because of regularization term)
        ...
        for _ in range(self.n_init):
            ...
            self.centroids_x = x[self.idx, :] + 1e-6  # white noise to avoid numerical instability
            self.centroids_a = a[self.idx, :] + 1e-6
            self._centroids_x = deepcopy(self.centroids_x)  # An attribute to save init centroids for further analysis
            self._centroids_a = deepcopy(self.centroids_a)
